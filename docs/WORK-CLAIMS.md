...
| WC-002 | GitHub produces the static artifacts consumed by Cloudflare Pages. | Implemented; production verification pending | build scripts, Cloudflare PR previews and CI | A commit-matched production deployment from `main` must still pass after the authentication correction merge. |
...
| WC-046 | The staging harness exposes PKCE magic-link authentication, onboarding, contact, realtime messaging, signed portrait, block, report, feedback and cleanup controls. | Implemented and artifact-validated | application and validator | Behavioural proof is pending on Cloudflare Pages. |
...
| WC-050 | Supabase Auth Site URL and allow-list use the canonical Cloudflare Pages URL. | Demonstrated remotely | protected run `30698614914` accepted and verified the URL patch before the unsupported template step failed | The full protected workflow must rerun successfully after removal of the unsupported template operation. |
...
| WC-052 | The canonical staging URL is `https://rendezvue-private-preview.pages.dev/`. | Decision accepted; deployment pending | issue #35 and roadmap v2.1 | Must be confirmed by fetching commit-matched deployment metadata. |
...
| WC-056 | The Cloudflare browser accepts a one-time PKCE code but rejects implicit access/refresh token fragments. | Implemented and artifact-validated | build metadata, source checks and validator | Controlled remote callback evidence remains. |
