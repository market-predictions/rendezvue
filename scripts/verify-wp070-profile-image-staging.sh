...
const query = `
  ...
  select
    ...
    to_regprocedure('public.register_prepared_portrait(uuid,text,text,text,numeric,numeric,numeric,integer,integer,text[])') is not null as register_rpc_present,
    has_function_privilege('anon', 'public.register_prepared_portrait(uuid,text,text,text,numeric,numeric,numeric,integer,integer,text[])', 'EXECUTE') as anon_can_register,
    has_function_privilege('authenticated', 'public.register_prepared_portrait(uuid,text,text,text,numeric,numeric,numeric,integer,integer,text[])', 'EXECUTE') as authenticated_can_register,
    exists (
    ...
    (
      ...
      where treatment in ('normalized-source-webp', 'prepared-card-4x5-webp', 'prepared-avatar-square-webp')
...
if (integer(row.prepared_column_count) !== 13) throw new Error('prepared portrait column contract is incomplete');
