-- WP-074B: remap the active participant portrait ladder without reinterpreting historical pixels.
-- Active new-write IDs become unfiltered, softFocus (Natural), warmVeil (Soft private), morePrivate.
-- Historical natural/monoMist/privacyMax values remain schema-readable but cannot be newly registered.
...
alter table public.privacy_portraits
  ...
  add constraint privacy_portraits_filter_id_check check (
    ...
    or privacy_filter_id in (
      'unfiltered', 'natural', 'softFocus', 'warmVeil', 'morePrivate',
      'monoMist', 'privacyMax'
    )
  );
...
begin
  ...
  if v_filter is null or v_filter not in ('unfiltered', 'softFocus', 'warmVeil', 'morePrivate') then
